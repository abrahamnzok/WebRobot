...
object UrlProcessor extends UrlTools{
	...
	def fetch(url:String): Html={
	  // Use HtmlCleaner to obtain a valid (X)Html document 
	   ...
	   try {
		   val rootNode = cleaner.clean(new URL(url),"ISO-8859-1")
		   tag2Html(rootNode)
	   } catch{
	   ...
	   tn match {
	     case tn: TagNode => {
	     	...
	     	(tn.getName,children) match{
	     		case ("html",_::_::Nil) => Tag("html",List(),List(tag2Html(children.get(0)),tag2Html(children.get(1))))
	     		case ("title",_)=> Tag("title",List(),List(Text(tn.getText.toString)))
	     		case ("meta",_) =>{      // to avoid having null objects in place of a string
	     			val content=tn.getAttributeByName("content")
	     			if (content==null) Tag("meta",List(),List()) 
	     			else Tag("meta",List(("content",content)),List())
	     		}	     		 
	     		case ("a",_) => // to avoid having null objects in place of a string
	     		  val href=tn.getAttributeByName("href")
	     		  if (href!=null) Tag("a",List(("href",href)),children.map(tag2Html(_)))
	     		  else Tag("a",List(),children.map(tag2Html(_)))
	     		case (name,_) => Tag(name,List(),children.map(tag2Html(_)))
	     	}
	     }
